...
fun ConverterHomeScreen(
    ...
    val cleanInputText = inputText.trim()
    val conversionResponse = if (cleanInputText.isBlank()) {
        null
    } else {
        UnitConversionCatalog.convertValue(
            inputText = cleanInputText,
                ...
                item {
                    ConversionSelectionSection(
                        ...
                        onInputChanged = { newInput ->
                            inputText = sanitizeNumberInput(newInput)
                        }
                    )
                }
...
private fun HeaderSection(totalCategories: Int) {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Text(
            text = "Selecciona una categoria, escribe un valor y revisa el resultado.",
    ...
    Card(
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                placeholder = {
                    Text(text = "Ejemplo: 100")
                },
                supportingText = {
                    val message = when {
                        inputText.isBlank() -> "Puedes usar punto o coma decimal."
                        conversionResponse is ConversionResponse.Error -> conversionResponse.message
                        else -> "Valor listo para convertir."
                    }

                    Text(text = message)
                },
                isError = conversionResponse is ConversionResponse.Error,
                singleLine = true,
...
private fun ConversionResultCard(
    ...
    val containerColor = when (conversionResponse) {
        ...
        is ConversionResponse.Success -> MaterialTheme.colorScheme.tertiaryContainer
        else -> MaterialTheme.colorScheme.surface
    }
    val contentColor = when (conversionResponse) {
        is ConversionResponse.Error -> MaterialTheme.colorScheme.onErrorContainer
        is ConversionResponse.Success -> MaterialTheme.colorScheme.onTertiaryContainer
        ...
        Column(
            ...
            when (conversionResponse) {
                is ConversionResponse.Success -> {
                    ...
                    Text(
                        text = buildResultDescription(result.category.id, result.fromUnit.name, result.toUnit.name),
                        style = MaterialTheme.typography.bodyMedium,
                        color = contentColor
                    )
                }
...
private fun buildResultDescription(
    ...
    return if (categoryId == "moneda") {
        "$fromUnitName a $toUnitName con tasa fija de practica."
    } else {
        "$fromUnitName a $toUnitName"
    }
}
